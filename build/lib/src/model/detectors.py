...
class EnsambleDetector(Detector):
    def __init__(self, **kwargs):
        """Ensamble detector for anomaly detection, uses part of ADTK library
        and a custom Gaussian Slider.
        """
        super().__init__(**kwargs)
...
class StartingDetector(Detector):
    def __init__(self, **kwargs):
        """Ensamble detector for anomaly detection, uses part of ADTK library
        and a custom Gaussian Slider.
        """
        super().__init__(**kwargs)
        self.level_shift_detector = ExternalDetector(
            detector=LevelShiftAD(c=10.0, side="positive", window=5)
        )
        self.persisent_detector = ExternalDetector(
            detector=PersistAD(c=10.0, side="positive")
        )
